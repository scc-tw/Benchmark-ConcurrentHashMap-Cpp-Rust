...
Implement two C++ benchmark executables (`bench-phmap` and `bench-libcuckoo`) with CLI argument parsing, all 10 workload scenarios, CSV output, latency sampling, and memory measurement.

## Prerequisites

- Phase 1 complete (CMake builds)
- Phase 2 complete (all 9 common headers)
- parallel-hashmap and libcuckoo fetched via CMake
...
│                    bench-phmap.cpp                          │
├─────────────────────────────────────────────────────────────┤
│  CLI Parser  │  Scenario Runner  │  CSV Writer  │  Main    │
└──────┬───────┴────────┬──────────┴──────┬───────┴────┬─────┘
       │                │                 │            │
       ▼                ▼                 ▼            ▼
┌──────────┐  ┌─────────────────┐  ┌──────────┐  ┌──────────┐
│ config.h │  │ worker.h        │  │ timing.h │  │ numa.h   │
│ prng.h   │  │ scenarios (10)  │  │ memory.h │  │ hasher.h │
└──────────┘  └─────────────────┘  └──────────┘  └──────────┘
```
...
### Step 3.1: Define CLI Arguments Structure

**File:** `cpp-impl/bench-phmap.cpp`

**Tasks:**
- [ ] Define BenchConfig struct for all CLI arguments
...
### Step 3.2: Implement CLI Argument Parser

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement parse_args() function
- [ ] Handle --scenario, --threads, --mapsize, --repeats, --seed
- [ ] Handle --pinning, --output, --verify-prng, --verify-hash
...
### Step 3.3: Define Map Type with SpinLock

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Define ConcurrentMap type alias
- [ ] Use parallel_flat_hash_map with N=4 (16 submaps)
- [ ] Use FastHasher and SpinLock

**Implementation:**
```cpp
// Concurrent map type: 16 submaps with spinlock
...
### Step 3.4: Implement Verification Functions

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement verify_prng() - print first 10 values
- [ ] Implement verify_hash() - hash known keys
...
### Step 3.5: Implement Pre-population Function

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement pre_populate() to fill map with M items
- [ ] Use sequential keys [0, M)
...
### Step 3.6: Implement Worker Thread Functions for Each Scenario

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement worker function for insert_only
- [ ] Implement worker function for read_only
- [ ] Implement worker function for read_majority (parameterized)
- [ ] Implement worker function for balanced
- [ ] Implement worker function for zipfian
- [ ] Implement worker function for resize_stress
- [ ] Implement worker function for sequential_keys
- [ ] Implement worker function for random_keys

**Implementation:**
```cpp
// Scenario 1: Insert-only (100% writes, unique keys per thread)
void* worker_insert_only(void* arg) {
    ...
    uint64_t base_key = static_cast<uint64_t>(ctx->thread_id) * ctx->ops_per_thread;
...
void* worker_read_only(void* arg) {
    WorkerContext* ctx = static_cast<WorkerContext*>(arg);
    ConcurrentMap* map = static_cast<ConcurrentMap*>(ctx->map);
    ...
    for (uint64_t i = 0; i < ctx->ops_per_thread; ++i) {
        ...
        uint64_t key = rand_range(&ctx->prng_state, map_size);
...
void* worker_read_majority(void* arg) {
    ...
    for (uint64_t i = 0; i < ctx->ops_per_thread; ++i) {
        ...
        uint64_t key = rand_range(&ctx->prng_state, map_size);
        int op_type = static_cast<int>(rand_range(&ctx->prng_state, 100));
        ...
        if (op_type < read_pct) {
            ...
            map->insert_or_assign(key, key + 1);
        }
...
void* worker_balanced(void* arg) {
    ...
    uint64_t map_size = map->size();
    uint64_t write_base = map_size;  // New keys start after existing
    auto t_start = get_time();
    ...
    for (uint64_t i = 0; i < ctx->ops_per_thread; ++i) {
        ...
        if (op_type < 50) {
            ...
            // Write new key
            uint64_t key = write_base + rand_range(&ctx->prng_state, map_size);
            map->insert({key, key});
        }
...
void* worker_resize_stress(void* arg) {
    WorkerContext* ctx = static_cast<WorkerContext*>(arg);
    ConcurrentMap* map = static_cast<ConcurrentMap*>(ctx->map);
...
void* worker_sequential_keys(void* arg) {
    WorkerContext* ctx = static_cast<WorkerContext*>(arg);
    ConcurrentMap* map = static_cast<ConcurrentMap*>(ctx->map);
...
void* worker_random_keys(void* arg) {
    WorkerContext* ctx = static_cast<WorkerContext*>(arg);
    ConcurrentMap* map = static_cast<ConcurrentMap*>(ctx->map);
    ...
    for (uint64_t i = 0; i < ctx->ops_per_thread; ++i) {
        ...
        uint64_t key = xorshift64star(&ctx->prng_state);
        map->insert({key, key});
...
### Step 3.7: Implement Latency Statistics Calculation

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Define LatencyStats struct
- [ ] Implement compute_latency_stats() for percentiles
...
### Step 3.8: Implement CSV Output Functions

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement print_csv_header()
- [ ] Implement print_csv_row() with all fields
...
void print_csv_row(FILE* out,
    ...
    fprintf(out, "%s,parallel-hashmap,2.0.0,%s,%d,%lu,%lu,%d,%lu,%s,"
...
### Step 3.9: Implement Single Trial Runner

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Implement run_trial() that orchestrates one benchmark run
- [ ] Create threads, barrier, run workers, collect results
...
### Step 3.10: Implement Main Function

**File:** `cpp-impl/bench-phmap.cpp` (append)

**Tasks:**
- [ ] Parse CLI args
- [ ] Handle verify-prng and verify-hash modes
...
int main(int argc, char** argv) {
    ...
    // Generate run ID
    char run_id[64];
    snprintf(run_id, sizeof(run_id), "phmap_%s_%dt_%lum",
...
### Step 3.11: Create bench-libcuckoo.cpp

**File:** `cpp-impl/bench-libcuckoo.cpp`

**Tasks:**
- [ ] Copy structure from bench-phmap.cpp
- [ ] Change include to libcuckoo/cuckoohash_map.hh
- [ ] Change map type to cuckoohash_map
- [ ] Adapt API calls (find → find(), insert, update_fn)

**Key differences from phmap:**
```cpp
// Include
#include <libcuckoo/cuckoohash_map.hh>

// Map type (libcuckoo uses different template params)
using ConcurrentMap = libcuckoo::cuckoohash_map<uint64_t, uint64_t, FastHasher>;

// Insert
map.insert(key, value);

// Find (different signature - uses output param)
uint64_t val;
bool found = map.find(key, val);

// Update (uses update_fn)
map.update_fn(key, [](uint64_t& v) { v += 1; });

// Or insert_or_assign equivalent
map.upsert(key, [](uint64_t& v) { v += 1; }, value);
```

**Implementation notes:**
- libcuckoo's `find()` takes output parameter, not iterator
- Use `upsert()` for insert-or-update operations
- Use `update_fn()` for in-place updates
- Reserve capacity differently: `map.reserve(n)`

---

### Step 3.12: Update cpp-impl/CMakeLists.txt

**File:** `cpp-impl/CMakeLists.txt`

**Tasks:**
- [ ] Ensure both executables have correct flags
...
### Step 3.13: Build and Verify

**Tasks:**
- [ ] Rebuild project
- [ ] Verify both executables compile
- [ ] Run verification tests
- [ ] Run simple benchmark to verify output
...
# Verify PRNG
./cpp-impl/bench-phmap --verify-prng --seed 42
./cpp-impl/bench-libcuckoo --verify-prng --seed 42

# Verify hash
./cpp-impl/bench-phmap --verify-hash
./cpp-impl/bench-libcuckoo --verify-hash

# Quick benchmark test
./cpp-impl/bench-phmap --scenario insert_only --threads 4 --mapsize 100000 --repeats 3

./cpp-impl/bench-libcuckoo --scenario insert_only --threads 4 --mapsize 100000 --repeats 3
```

---

### Step 3.14: Test All Scenarios

**Tasks:**
- [ ] Test each scenario with small parameters
...
for s in $SCENARIOS; do
    echo "Testing $s..."
    ./cpp-impl/bench-phmap --scenario $s --threads 2 --mapsize 10000 --repeats 2
done
```
...
- [ ] bench-phmap compiles without warnings
- [ ] bench-libcuckoo compiles without warnings
- [ ] --verify-prng produces expected output
- [ ] --verify-hash produces expected output
- [ ] All 9 scenarios run without errors
- [ ] CSV output matches specification format
- [ ] Latency statistics are computed correctly
- [ ] Memory measurements are populated
- [ ] Thread pinning works (verify with htop or taskset)

## Files Created

| File | Purpose | Lines (approx) |
|------|---------|----------------|
| `cpp-impl/bench-phmap.cpp` | parallel-hashmap benchmark | ~600 |
| `cpp-impl/bench-libcuckoo.cpp` | libcuckoo benchmark | ~600 |
| `cpp-impl/CMakeLists.txt` | Build rules | ~15 |

## API Differences Summary

| Operation | parallel-hashmap | libcuckoo |
|-----------|------------------|-----------|
| Include | `<parallel_hashmap/phmap.h>` | `<libcuckoo/cuckoohash_map.hh>` |
| Type | `parallel_flat_hash_map<K,V,H,E,A,N,M>` | `cuckoohash_map<K,V,H>` |
| Insert | `map.insert({k, v})` | `map.insert(k, v)` |
| Find | `auto it = map.find(k)` | `map.find(k, out_val)` |
| Update | `map.insert_or_assign(k, v)` | `map.upsert(k, fn, v)` |
| Reserve | `map.reserve(n)` | `map.reserve(n)` |
| Size | `map.size()` | `map.size()` |

## Next Phase

After verification, proceed to **Phase 4: Rust Benchmark** to implement `bench-dashmap` with identical functionality.
